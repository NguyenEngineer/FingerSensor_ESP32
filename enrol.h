...
byte rowPins[ROWS] = {13, 12, 14, 27}; //connect to the row pinouts of the keypad hang
byte colPins[COLS] = {26, 25, 33, 32}; //connect to the column pinouts of the keypad cot
...
void loop_e()                     // run over and over again
{
  ...
  id = readnumber();
  if (id == 0) {// ID #0 not allow, try again!
    return;
  }
